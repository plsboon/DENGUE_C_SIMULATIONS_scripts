...
parser.add_argument("-l","--labels", nargs='+', metavar='L', help="list of labels for input files", required=True)
parser.add_argument("-o","--outfile", nargs='?', default="block_SASA.svg", help="output file name .svg")
parser.add_argument("-c", "--crystal",nargs=1, help="Radius of gyration for crystal structure .xvg format")
parser.add_argument("-log", nargs="?", default="block_SASA.log", help="log file containing hopefully useful information")
...
if len(args.infiles) != len(args.labels):
	...
	plt.ylabel("Volume (Angstroms^3)",size=30)
	...
	for i in range(len(args.infiles)):
		...
		plt.axis([0,max(blocks)+200000,0,1000])
